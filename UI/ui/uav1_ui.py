...
class MessageListenerThread(Thread):
    ...
    def run(self):
        ...
        def remove_noise(ciphertext_with_noise):
            noise_freq = int(ciphertext_with_noise[-1])
            decoded = ""
            i, j = 0, 0
            while i < len(ciphertext_with_noise) - 1:
                if i > 0 and (i - j) % ((j + 1) * noise_freq) == 0:
                    ...
                    decoded += ciphertext_with_noise[i]
                    i += 1
            return decoded
        ...
        try:
            ...
            while self.running:
                ...
                Index=Index+1
                self.text_widget.after(0, self.update_text, "密文："+origin)
                self.text_widget.after(0, self.update_text, "明文："+message)
